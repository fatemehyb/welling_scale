...
def main(dis_volume,inta_volume,sig_volume,ossic_volume,fac_volume,Inn_volume,dis_volume_add,Normall,Spacing,Low,High,Tegmen_volume):
    # time.time()
    startTime = time.time()
...
#########################################################################################################
    ...
    # origin=input_volume.GetOrigin()
    try:
        myTegmen=sitk.GetArrayFromImage(Tegmen_volume)
    except:
        myTegmen=itk.GetArrayFromImage(Tegmen_volume)
    ...
    myInner[np.where(myInner>0)]=255
    myFacial[np.where(myFacial>0)]=255
    myTegmen[np.where(myTegmen>0)]=255
    del(intact_volume)

    roi2=myInner+myFacial+myTegmen
    roi2[np.where(roi2>0)]=255
    ...
    r1=rmin_s-1000*abs(The_Normal2[0])-10
    r2=rmax_s+1000*abs(The_Normal2[0])+10
    c1=cmin_s-1000*abs(The_Normal2[1])-10
    c2=cmax_s+1000*abs(The_Normal2[1])+10
    z1=zmin_s-1000*abs(The_Normal2[2])-10
    z2=zmax_s+1000*abs(The_Normal2[2])+10
    if r1<=0:
        r1=0
    if c1<=0:
        c1=0
    ...
    sub_res2[int(r1):int(r2),int(c1):int(c2),int(z1):int(z2)]=sub_res[int(r1):int(r2),int(c1):int(c2),int(z1):int(z2)]
    # sub_res2[rmin_s:rmax_s,cmin_s:cmax_s,zmin_s:zmax_s]=sub_res[rmin_s:rmax_s,cmin_s:cmax_s,zmin_s:zmax_s]
    ...
    original_segmented_filled[np.where(original_segmented_filled>0)]=255
    original_segmented_filled[np.where(original_segmented_filled<=0)]=0
    # del(region2)
    ...
    t_list1,rgb_image,grid1,grid2=prc.main3_extended(original_segmented_filled,d_shape,h_shape,w_shape,CameraEye3,center_volume2)
    # t_list1,rgb_image,grid1,grid2=prc.main3_extended(original_segmented_filled,d_shape,h_shape,w_shape,CameraEye3,center_volume2)
...
####################################################################################################################################
    ...
    final_list_1=eva.evaluate2_2(t_list1)
    ...
    ##########################################################################
    # write_string2=os.path.join(".\\"+ "\\" +x[-2]+"_"+x[-1]+"_"+"volume2"+ "." + "nrrd")
    # output_volume=np.zeros((d,h,w))
    # output_volume=sitk.GetImageFromArray(np.asarray(original_segmented_filled,dtype=np.float))
    #
    # output_volume=sitk.Cast(output_volume,sitk.sitkFloat64)
    ...
    #
    # write_string2=os.path.join(".\\"+ "\\" +x[-2]+"_"+x[-1]+"_"+"volume22"+ "." + "nrrd")
    # output_volume=np.zeros((d,h,w))
    # output_volume=sitk.GetImageFromArray(np.asarray(input_volume2,dtype=np.float))
    #
    # output_volume=sitk.Cast(output_volume,sitk.sitkFloat64)
    # output_volume.SetSpacing(np.asarray(spacing))
    #
    ...
    ###################################################################################
    print ("Execution time: %8.2f seconds." % (time.time() - startTime))
    res1=cluster.main(write_string)
...
#####################################################################################################
    ...
    try:
        ...
        t_list2,rgb_image,grid1,grid2=prc.main3_extended(test2_region,test2_region.shape[0],test2_region.shape[1],test2_region.shape[2],CameraEye3,center_volume2)
        del(grid1)
        del(grid2)
        del(rgb_image)
    except:
...
####################################################################
    ...
    try:
        #find the the index of the voxels exposed to overhang problem for both volumes
        final_list_2=eva.evaluate2_2(t_list2)
