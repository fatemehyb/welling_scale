...
# import parallel_ray_casting_2d as prc
...
# import math
import glm
import call_normal_computation
# import skimage
...
# parser.add_argument("-args0", type = str, default = (('D:\Fatemeh\\UWO_CASES\\UWO_CASE_1512R\\sonny3')), help = "dissected image address")
# parser.add_argument("-args1",type=str, default=('D:\Fatemeh\\UWO_CASES\\UWO_CASE_1512R\\1512R_154um_DICOM'),help="intact image address")

parser.add_argument("-args0", type = str, default = (('D:\evan_campton\\fatemeh_no_overhang4')), help = "dissected image address")
...
w1 = myOssicles.shape[2]
h1 = myOssicles.shape[1]
d1 = myOssicles.shape[0]
...
#########################################################################
#find
region_of_interest_small=sub_res
...
dd_shape=region_test_center.shape[0]
hh_shape=region_test_center.shape[1]
ww_shape=region_test_center.shape[2]

############center of dissected area################################
center_volume=((dd_shape/2)+rmin3,(hh_shape/2)+cmin3, (ww_shape/2)+zmin3)
...
del(rgb_image)
del(roi3)
region2=region2[rmin:rmax,cmin:cmax,zmin:zmax]
try:
    t_list2,rgb_image,grid1,grid2=prc.main3_editted(region2,region2.shape[0],region2.shape[1],region2.shape[2],center_volume2,center_ossicles)
    del(grid1)
    del(grid2)
    del(rgb_image)
except:
...
blobs_2=np.zeros((input_volume2[rmin:rmax,cmin:cmax,zmin:zmax]).shape,dtype=bool)
...
blobs2[(int)(rmin):(int)(rmax),(int)(cmin):(int)(cmax),int(zmin):int(zmax)]=blobs_2
print(np.sum(blobs2))
...
os.path.split(args.args0)[0]
x=args.args0.split("\\")
print(x[-2]+"\\"+x[-1])
write_string=os.path.join("D:\Results"+ "\\" +x[-2]+"_"+x[-1]+ "." + "nrrd")
...
# write_string=str("\\\\samba.cs.ucalgary.ca\\fatemeh.yazdanbakhsh\Documents\medical_imaging\\results\Totlavolume_3D_9_3_oct_19_1_test.nrrd")
