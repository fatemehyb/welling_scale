...
def main3_extended(f_data,width,height,depth,cameraEye,volume_centre):
    ...
    a_ray = (pixelPositionss2 - [cameraEye.x,cameraEye.y,cameraEye.z])
    a_ray=np.transpose(a_ray)
    ray=a_ray/np.sqrt((a_ray[0]**2+a_ray[1]**2+a_ray[2]**2))
    del(a_ray)

    n=((magnitude2((pixelPositionss2 - [cameraEye[0],cameraEye[1],cameraEye[2]]))-(max(width,height,depth)/2)))
    del(pixelPositionss2)
    # n=0
    # nn=(n+max(width,height,depth))/stepSize
    nn=(n.max()+max(width,height,depth))
    # nn=(1000+max(width,height,depth))
    # rayy=np.zeros((1,int(nn-n+1)))
    # rayy=np.zeros((1,int(nn)-int(n)),dtype=glm.vec3)
    # rayy_n_1=np.zeros((1,int(nn)-int(n)+1),dtype=glm.vec3)
    # rayy=np.zeros((grid2.shape))
    # rayy_n_1=np.zeros((grid2.shape))
    n=int(n.min())
...
##########################################################################################
    ...
    c1[0]=np.transpose(grid2.shape[0]*[grid2.shape[1]*[np.asarray(range(int(n),int(nn)))]])
    c1[1]=np.transpose(grid2.shape[0]*[grid2.shape[1]*[np.asarray(range(int(n),int(nn)))]])
    c1[2]=np.transpose(grid2.shape[0]*[grid2.shape[1]*[np.asarray(range(int(n),int(nn)))]])
    del(ray)
    # rayy=np.transpose(cam)+np.dot(np.dot(np.transpose(np.asarray(a1)),(np.asarray(b1))),np.transpose(np.asarray(c1)))
    rayy=np.asarray(cam)+np.multiply(np.multiply((np.asarray(a1)),(np.asarray(b1))),np.asarray(c1))
    del(a1)
    ...
    rayy[np.where(rayy<0)]=None
    rayy[0][np.where(rayy[0]>width)]=None


    rayy[1][np.where(rayy[1]>height)]=None

    rayy[2][np.where(rayy[2]>depth)]=None


    rayy_n_1=np.zeros((np.asarray(rayy).shape))
    # rayy_n_1=(rayy_n_1)
    rayy_n_1[:,:,:,1:]=rayy[:,:,:,0:-1]
    # rayy_n_1[:][:][1:]=(rayy)[:][:][:-1]
    ...
    a=rayy[0][[[np.where([~(np.isnan(rayy[0]))])]][0][0][1:4]]
    a=[int(j) for j in a]
    a2=rayy[1][[[np.where([~(np.isnan(rayy[1]))])]][0][0][1:4]]
    a2=[int(j) for j in a2]

    a3=rayy[2][[[np.where([~(np.isnan(rayy[2]))])]][0][0][1:4]]
...
###############################################################################################
    ...
    an=rayy_n_1[0][[[np.where([~(np.isnan(rayy_n_1[0]))])]][0][0][1:4]]
    an=[int(j) for j in an]
    a2n=rayy_n_1[1][[[np.where([~(np.isnan(rayy_n_1[1]))])]][0][0][1:4]]
    a2n=[int(j) for j in a2n]

    a3n=rayy_n_1[2][[[np.where([~(np.isnan(rayy_n_1[2]))])]][0][0][1:4]]
    ...
    #     for j in a2:
    #         for k in a3:
    #             rayy_L[i][j][k]=f_data[i][j][k]

    # np.where(f_data[a1][a2][a3]==255)
    ...
    # t_list=np.concatenate((np.transpose(in_list2),np.transpose(out_list2)))
    ...
    # out_list2=(dfs).to_numpy()
    t_list=np.asarray(t_list,dtype=object)
    t_list[np.where(np.asarray(rayy_test)==-255)]=np.transpose(out_list2)
    # t_list[np.where(np.asarray(rayy_test)==-255)]=out_list2
