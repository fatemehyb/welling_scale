...
segmented_area=np.zeros((dissected_array_thr.shape))
segmented_area[rmin3:rmax3,cmin3:cmax3,zmin3:zmax3]=dissected_array_thr[rmin3:rmax3,cmin3:cmax3,zmin3:zmax3]
...
segmented_area=dissected_array_thr
print(segmented_area.shape)
...
mesh=mlab.triangular_mesh([vert[0] for vert in verts],
                             [vert[1] for vert in verts],
                             [vert[2] for vert in verts],
                            faces,color=(0.0,1.0,0.0))
mesh.mlab_source.dataset.cell_data.scalars = np.ones(faces.size)*256
...
# mesh.mlab_source.scalars = np.ones(faces.size)*256
# mlab.pipeline.surface(mesh,color=(1.0,0.0,0.0))
fig=mlab.figure('complete image2')
segmented_area=np.zeros((dissected_array_thr.shape))
segmented_area[rmin3:rmax3,cmin3:cmax3,zmin3:zmax3]=dissected_array_thr[rmin3:rmax3,cmin3:cmax3,zmin3:zmax3]
segmented_area=segmented_area+facial_volume+sigmoid_volume
verts, faces, normals, values = marching_cubes_lewiner(segmented_area, 0, spacing)
mesh2=mlab.triangular_mesh([vert[0] for vert in verts],
                             [vert[1] for vert in verts],
                             [vert[2] for vert in verts],
                            faces,color=(1.0,0.0,0.0))
mesh2.mlab_source.dataset.cell_data.scalars = np.ones(faces.size)*10
for i in np.unique(mesh.mlab_source.triangles):
    mesh2.mlab_source.dataset.cell_data.scalars[int(i)] =256
# mesh2.mlab_source.dataset.cell_data.scalars[np.where(np.unique(mesh.mlab_source.triangles))] =256
# mesh2.mlab_source.dataset.cell_data.scalars = np.ones(faces.size)*0
mesh2.mlab_source.dataset.cell_data.scalars.name = 'Cell data'

# mlab.show()
mesh3= mlab.pipeline.set_active_attribute(mesh2,cell_scalars='Cell data')
# mlab.pipeline.surface(mesh2,color=(1.0,1.0,0.0))
# mesh2.mlab_source.scalars = np.ones(faces.size)*0
mlab.pipeline.surface(mesh3)
# mlab.title("part of intact")
# fig = mlab.figure("part of intact")
mlab.show()
